...
ProcessManager::ProcessManager()
  ...
  poller_.add_action(Poller::Action(signal_fd_.fd(), Direction::In, [this]() {
...
pid_t ProcessManager::run_as_child(const string& program,
  ...
  auto child = ChildProcess(program, [&program, &prog_args, &log_path]() {
...
int ProcessManager::run(const string& program, const vector<string>& prog_args,
                        const callback_t& callback) {
...
Result ProcessManager::handle_signal(const signalfd_siginfo& sig) {
        ...
        if (child.terminated()) {
          if (child.exit_status() != 0) {
            ...
            if (callback_it != error_callbacks_.end()) {
              ...
              throw runtime_error("ProcessManager: PID " +
                                  to_string(it->first) + " exits abnormally");
            }
          }
          ...
          if (not child.running()) {
            ...
            if (callback_it != error_callbacks_.end()) {
              ...
              child_processes_.clear();
              throw runtime_error("ProcessManager: PID " +
                                  to_string(it->first) + " is not running");
            }
          }
  ...
  default:
    cerr << "ProcessManager: unknown signal " + to_string(sig.ssi_signo) + "\n";
    child_processes_.clear();
    return {ResultType::Exit, EXIT_FAILURE};
  }
